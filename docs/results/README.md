...
- [ws30-replay-gonogo.md](ws30-replay-gonogo.md) — **WS3.0: replay cast-conditioned value GO** (DSK full replay n=1,011,949 games, $0 local; split-half ρ 0.976 beats matched-n deck_value 0.944 and WS1.1 ref 0.909; Sp vs GIH 0.637 — new signal; face-plausible with documented cast-selection confound: tricks rank high, desperation diggers rank low; WS3.1 now unlocked pending confound-control design). Raw: [ws30-replay-gonogo.json](ws30-replay-gonogo.json).
- [ws31-cast-adjudicator.md](ws31-cast-adjudicator.md) — **WS3.1 Stage 1: RULER-ROBUST** (adjudicator swap: WS1.2 outcome eval re-run with three cast-β rulers — full cast, early-cast turn≤6, normalized-cast — on same 800 DSK drafts, $0 local; confound-mitigation study first: early-cast partially de-confounds (Turn Inside Out #7→#13, Glimmerburst #173→#56), normalized-cast amplifies the confound (Turn Inside Out→#1) and is rejected as mitigation; beats-human % is the cross-ruler comparable (sigma-scale margins NOT comparable due to saturation at 0.92–1.00 vs 0.63 under deck_value); model beats humans in **70–82% of drafts** under all cast rulers incl. confound-mitigated; deck_value had flattered model (92–98%); gih_greedy > model universal; Stage 2 teacher re-test declined — WS1.3 subsumption logic + budget). Raws: [ws31/](ws31/).
- [ws24-coldstart.md](ws24-coldstart.md) — **WS2.4: cold-start LLM teacher — first day-0 number** (content-only Claude Sonnet teacher, 286 DSK cards scored 0–10 by 10-agent workflow with adversarial verify, 4 fixes; CachedTeacher bypass — no API key at eval time; $0 pod; ~240k agent tokens; agent-claude closes **19/21/19% of oracle−baseline WR-agree gap** across 3 seeds (mean ~20%, SD 0.9 pp) at alpha=4; heuristic floor closes −4% at best alpha (Sp=0.081); Sp(agent-claude, real GIH)=0.463, n=272; ⚠ UPPER BOUND — DSK predates scoring-agent cutoff, training-data leak possible; single holdout, 4-set corpus). Raws: [ws24/](ws24/).
- [ws22-llm-tags.md](ws22-llm-tags.md) — **WS2.2: LLM-annotated card tags BELOW-GATE (twice-replicated real effect)** (14 structured dimensions generated by 50-agent haiku workflow + sonnet adversarial verifier, <4% error rate; v1 79% train coverage: top-1 +0.0038 t=2.06, WR-agree +0.0062 t=3.78; v2 100% coverage: top-1 +0.0051 t=1.99, WR-agree +0.0060 t=3.03; gains concentrate in control's two weakest holdouts WOE +0.019, MKM +0.013; full-coverage dilution hypothesis refuted; real but under +0.01 bar — not adopted; stop-condition triggered; 1,682 tagged cards are a durable HF asset).
